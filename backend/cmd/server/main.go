...
func setupRoutes(engine *gin.Engine, db *gorm.DB, cfg *config.Config) {
	...
	// ---- Auth (public rate limit: 5000 req / 5 min per IP) ----
	authLimiter := middleware.NewRateLimiter(5000, 300)
	...
	api.PUT("/admin/mentors/:id", adminMentorHandler.Update)
	...
	api.DELETE("/admin/products/:id", productHandler.Delete)
