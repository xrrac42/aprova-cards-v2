...
const AdminMentors: React.FC = () => {
  ...
  const slugTooShort =
    form.slug.trim().length > 0 && form.slug.trim().length < 3;
  ...
  const getFriendlyErrorMessage = (raw: string) => {
    ...
    if (
      message.includes('column "email" of relation "mentors" does not exist')
    ) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (logoFile) {
        ...
        if (uploadError)
          throw new Error(`Falha ao enviar a logo: ${uploadError.message}`);
      ...
      if (editing) {
        ...
        const res = await fetch(
          `${backendURL}/api/v1/admin/mentors/${editing.id}`,
          {
            method: "PUT",
      ...
      setErrorModalMessage(
        getFriendlyErrorMessage(error?.message || "Erro ao salvar mentor"),
      );
  ...
  const deleteMentor = async () => {
    ...
    try {
      const backendURL =
        import.meta.env.VITE_BACKEND_URL || "http://localhost:8080";
      const res = await fetch(
        `${backendURL}/api/v1/admin/mentors/${deleteModal.id}`,
        { method: "DELETE" },
      );
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
  ...
  return (
    <AdminLayout>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-6 flex items-center justify-between gap-3">
            ...
            >
              ...
              <span className="hidden sm:inline">Novo</span> Mentor
            </button>
          </div>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div
                        ...
                        style={{ backgroundColor: m.primary_color + "20" }}
                      >
                        <span
                          className="font-display font-bold"
                          style={{ color: m.primary_color }}
                        >
                          {m.name.charAt(0)}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-1 shrink-0">
                      <Link
                        to={`/admin/mentor/${m.id}`}
                        className="rounded-lg p-2 text-muted-foreground hover:bg-surface-hover hover:text-foreground transition-colors touch-manipulation"
                        title="Ver detalhes"
                      >
                      ...
                      <button
                        ...
                        className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors touch-manipulation"
                      >
                  ...
                  {m.slug ? (
                    ...
                    <div className="flex items-center gap-2 rounded-xl bg-destructive/10 px-3 py-2">
                      ...
                      <span className="text-xs text-destructive font-medium">
                        Sem slug — link de login indisponível
                      </span>
                      <button
                        onClick={() => openEdit(m)}
                        className="ml-auto text-xs text-destructive underline underline-offset-2 shrink-0"
                      >
                        Editar
                      </button>
                    </div>
      ...
      {showModal && (
        ...
        >
          <div
            className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-card p-6 space-y-3"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-display font-semibold text-foreground">
              {editing ? "Editar Mentor" : "Novo Mentor"}
            </h3>
            ...
            <div>
              ...
              />
              <p
                className={`mt-1 text-[11px] ${slugTooShort ? "text-destructive" : "text-muted-foreground"}`}
              >
                {slugTooShort
                  ? "Slug deve ter pelo menos 3 caracteres."
                  : "Gerado automaticamente a partir do nome (minimo 3 caracteres)."}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                Usado no login do painel mentor
              </p>
            </div>
        ...
        >
          ...
          >
            <h3 className="font-display text-lg font-semibold text-foreground">
              Nao foi possivel salvar o mentor
            </h3>
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setDeleteModal({ open: false, id: "", name: "" })
        }
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              ? Todos os produtos, disciplinas e cards vinculados serão
