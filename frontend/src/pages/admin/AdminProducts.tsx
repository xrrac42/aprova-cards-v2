...
const DuplicateModal: React.FC<DuplicateModalProps> = ({
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      const createRes = await fetch(`${backendURL}/api/v1/admin/products`, {
        method: "POST",
        headers: {
      ...
      if (!disciplines?.length) {
        setResult({ newProductId, disciplines: 0, cards: 0 });
        onSuccess(newProductId);
        return;
      }

      // 3. Create disciplines via Go API
      const newDiscIds: string[] = [];
      for (const disc of disciplines) {
        const discRes = await fetch(
          `${backendURL}/api/v1/admin/products/${newProductId}/disciplines`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name: disc.name, order: disc.order }),
          },
        );
        if (discRes.ok) {
          const discData = await discRes.json();
          newDiscIds.push(discData.data?.id || "");
        }
      }
      ...
      if (includeCards && newDiscIds.length === disciplines.length) {
        const BATCH = 500;

        for (let i = 0; i < disciplines.length; i++) {
          const srcDisc = disciplines[i];
          const destDiscId = newDiscIds[i];
          if (!destDiscId) continue;

          setStage(
            `Copiando cards: ${srcDisc.name} (${i + 1}/${disciplines.length})`,
          );
          ...
          while (true) {
            ...
            totalDuplicates += cards.length - uniqueCards.length;
              ...
              const { error: insertErr } = await supabase.from("cards").insert(
                uniqueCards.map((c) => {
                  ...
                  return {
                    ...
                    discipline_id: destDiscId,
                  };
                }),
              );
              if (!insertErr) totalCards += uniqueCards.length;
            }
          ...
          {result ? (
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mb-2">
                {result.disciplines} disciplina
                {result.disciplines !== 1 ? "s" : ""} e{" "}
                {result.cards.toLocaleString()} card
                {result.cards !== 1 ? "s" : ""} copiado
                {result.cards !== 1 ? "s" : ""}
                {result.duplicates
              ...
              <p className="text-xs text-amber-600 dark:text-amber-400 mb-6">
                O produto foi criado como <strong>Inativo</strong>. Ative
                manualmente quando estiver pronto.
              </p>
              <div className="flex gap-3">
                <button
                  onClick={onClose}
                  className="flex-1 rounded-xl border border-border py-2.5 text-sm font-medium text-foreground hover:bg-surface-hover transition-colors"
                >
                  Fechar
                </button>
                <button
                  onClick={() =>
                    navigate(`/admin/produto/${result.newProductId}`)
                  }
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer py-1">
                ...
                >
                  <span
                    className={`absolute top-0.5 h-5 w-5 rounded-full bg-foreground transition-transform ${includeCards ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
                <span className="text-sm text-foreground">
                  Incluir todos os cards
                </span>
              </label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  <Copy className="h-4 w-4" /> Clonar produto
                </button>
              </div>
            </div>
          )}
...
const AdminProducts: React.FC = () => {
  ...
  const toggleActive = async (id: string, active: boolean) => {
    const backendURL =
      import.meta.env.VITE_BACKEND_URL || "http://localhost:8080";
    const res = await fetch(`${backendURL}/api/v1/admin/products/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteProduct = async () => {
    if (!deleteModal.product || deleteConfirmation !== deleteModal.product.name)
      return;
    setDeleting(true);
    try {
      const backendURL =
      ...
      const res = await fetch(
        `${backendURL}/api/v1/admin/products/${deleteModal.product.id}`,
        {
          method: "DELETE",
        },
      );
      if (!res.ok && res.status !== 204) {
        ...
        throw new Error(err.error || "Erro ao apagar produto");
      }
      toast.success(
        `Produto "${deleteModal.product.name}" apagado com sucesso.`,
      );
      setDeleteModal({ open: false, product: null });
      setDeleteConfirmation("");
      load();
    } catch (err: any) {
      toast.error(err.message || "Erro ao apagar produto");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const filtered = products.filter((p) => {
    const matchSearch =
      ...
      p.access_code.toLowerCase().includes(search.toLowerCase());
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-6 flex items-center justify-between gap-3">
            ...
            <Link
              ...
              className="flex items-center gap-2 rounded-xl bg-primary px-3 sm:px-4 py-2.5 text-sm font-medium text-primary-foreground hover:opacity-90 transition-all shrink-0"
            >
              ...
              {filtered.map((p) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button
                        ...
                        className="rounded-lg p-1 text-muted-foreground hover:text-foreground transition-colors touch-manipulation"
                      >
                        {p.active ? (
                      ...
                      <Link
                        to={`/admin/produto/${p.id}`}
                        className="rounded-lg p-2 text-primary hover:bg-primary/10 transition-colors touch-manipulation"
                        title="Gerenciar"
                      >
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${p.active ? "bg-secondary/10 text-secondary" : "bg-destructive/10 text-destructive"}`}
                    >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              <div className="space-y-3 pt-2">
                <div className="rounded-xl bg-destructive/10 border border-destructive/20 p-4 text-sm space-y-2">
                  <p className="font-medium text-foreground">
                    Esta ação é irreversível. Será apagado:
                  </p>
                  <ul className="list-disc list-inside text-muted-foreground space-y-0.5 text-xs">
                    ...
                    <li>Todas as disciplinas e cards vinculados</li>
                    <li>Todos os acessos, progresso e sessões de alunos</li>
                  </ul>
                </div>
                <div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                deleteConfirmation !== deleteModal.product?.name || deleting
              }
            >
