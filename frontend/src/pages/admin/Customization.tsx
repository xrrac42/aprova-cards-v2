  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (creating) {
        ...
        const { error } = await supabase.from('mentors').update(mentorData).eq('id', selectedMentor.id);
        if (error) throw error;
      ...
      if (selectedMentor) {
        const updated = (freshMentors || []).find((m: any) => m.id === selectedMentor.id);
    ...
    <AdminLayout>
      <div className="p-4 md:p-8">
        <div className="mx-auto max-w-3xl">
          <h1 className="mb-6 font-display text-2xl font-bold text-foreground">Personalização Visual</h1>

          <div className="mb-6 flex flex-wrap gap-2">
            {mentors.map(m => (
              <button key={m.id} onClick={() => selectMentor(m)}
          ...
          {(creating || selectedMentor) && (
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-4 rounded-2xl border border-border bg-card p-6">
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">E-mail do Mentor</label>
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Senha do Mentor</label>
                ...
                <div>
                  ...
                  <div className="flex gap-3 items-center">
                    <input type="color" value={primaryColor} onChange={(e) => setPrimaryColor(e.target.value)} className="h-10 w-14 rounded-lg border-none cursor-pointer" />
                ...
                <div>
                  ...
                  <div className="flex gap-3 items-center">
                    <input type="color" value={secondaryColor} onChange={(e) => setSecondaryColor(e.target.value)} className="h-10 w-14 rounded-lg border-none cursor-pointer" />
                    <span className="font-mono text-sm text-muted-foreground">{secondaryColor}</span>
                  </div>
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Cor de Destaque</label>
                  <p className="mb-1.5 text-xs text-muted-foreground">Usada em botões de ação, badges e elementos interativos</p>
                  <div className="flex gap-3 items-center">
                    <input type="color" value={accentColor} onChange={(e) => setAccentColor(e.target.value)} className="h-10 w-14 rounded-lg border-none cursor-pointer" />
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Token Webhook Kiwify</label>
                  <input value={kiwifyToken} onChange={(e) => setKiwifyToken(e.target.value)}
                    placeholder="Cole aqui o token do webhook"
                    className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-foreground font-mono focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors" />
                  <p className="mt-1 text-xs text-muted-foreground">Após criar o webhook na Kiwify (Apps → Webhooks), copie o token gerado e cole aqui</p>
                </div>
                <button onClick={handleSave} disabled={loading || !name || !slug || !password}
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-border p-6" style={{ background: '#0a0a0f' }}>
                  <div className="flex flex-col items-center gap-4">
                    <div className="h-12 w-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: primaryColor + '20' }}>
                      <span className="font-display text-xl font-bold" style={{ color: primaryColor }}>{name?.charAt(0) || 'M'}</span>
                    </div>
                    <p className="font-display font-bold text-foreground">{name || 'Mentor'}</p>
                  ...
                  <div className="mt-6">
                    <div className="h-2 w-full rounded-full bg-surface">
                      <div className="h-2 rounded-full w-3/4" style={{ background: `linear-gradient(90deg, ${primaryColor}, ${secondaryColor})` }} />
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-4 gap-2">
                    <div className="rounded-lg py-2 text-center text-xs font-medium" style={{ backgroundColor: '#dc262620', color: '#dc2626' }}>Errei</div>
                    <div className="rounded-lg py-2 text-center text-xs font-medium" style={{ backgroundColor: '#f9731620', color: '#f97316' }}>Difícil</div>
                    <div className="rounded-lg py-2 text-center text-xs font-medium" style={{ backgroundColor: secondaryColor + '20', color: secondaryColor }}>Médio</div>
                    <div className="rounded-lg py-2 text-center text-xs font-medium" style={{ backgroundColor: primaryColor + '20', color: primaryColor }}>Fácil</div>
                  </div>
                </div>
              </div>
            </div>
