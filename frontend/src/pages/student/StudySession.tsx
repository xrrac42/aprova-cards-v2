...
import { saveToBackup } from "@/lib/study-backup";
...
let activeProgressCache = new Map<
  string,
...
const RATING_CONFIG = [
  {
    rating: "errei" as Rating,
    label: "Errei",
    colorClass: "bg-destructive/15 text-destructive",
  },
  {
    rating: "dificil" as Rating,
    label: "Difícil",
    colorClass: "bg-rating-hard/15 text-rating-hard",
  },
  {
    rating: "medio" as Rating,
    label: "Médio",
    colorClass: "bg-secondary/15 text-secondary",
  },
  {
    rating: "facil" as Rating,
    label: "Fácil",
    colorClass: "bg-rating-easy/15 text-rating-easy",
  },
];
...
const RatingButtons = memo(
  ({
    ...
    <div
      className="fixed bottom-0 left-0 right-0 z-20 px-4 shrink-0"
...
const StudySession: React.FC = () => {
  ...
  const updateStats = useCallback(
    (next: { reviewed: number; correct: number; incorrect: number }) => {
      activeStats = next;
  ...
  useEffect(() => {
    ...
    const isRestoredSession =
      activeSessionKey === sessionKey && activeCards.length > 0 && activeLoaded;
  ...
  useEffect(() => {
    ...
    const studyTimeSecs = activeStartTime
      ? Math.floor((Date.now() - activeStartTime) / 1000)
      : 0;
    ...
    navigate("/aluno/fim-de-sessao", {
      state: {
        stats: activeStats,
        disciplineName: activeDisciplineName,
        studyTimeSeconds: studyTimeSecs,
      },
    });
  ...
  const loadCards = async () => {
    try {
      ...
      const url = `${BACKEND_URL}/api/v1/student/study-cards?product_id=${session!.product_id}${isAll ? "" : `&discipline_id=${disciplineId}`}&mode=${studyMode}&new_limit=${newLimit}&t=${sessionTimestamp || Date.now()}`;
      ...
      const rawCards: RawStudyCard[] = json.data || [];
  ...
  const saveSession = useCallback(
    async (stats: { reviewed: number; correct: number; incorrect: number }) => {
      ...
      try {
        // supabase-js v2 NÃO lança exceção em falha — o erro vem no retorno
        ...
        if (error) {
          console.error("Erro ao salvar sessão:", error.message, error.details);
        }
      } catch (err) {
  ...
  const saveProgressBackground = useCallback(
    async (
      card: StudyCard,
      rating: Rating,
      correct: boolean,
      existingCounts: { correct_count: number; incorrect_count: number },
    ) => {
      ...
      const progressData = {
        ...
        incorrect_count: !correct
          ? existingCounts.incorrect_count + 1
          : existingCounts.incorrect_count,
      };
      ...
      try {
        ...
        if (error) {
          ...
          // Guarda no backup local; o syncBackup do StudentHome reenvia depois
          saveToBackup(s.email, s.product_id!, progressData);
        }
      } catch (err) {
  ...
  const handleRating = useCallback(
    (rating: Rating) => {
      ...
      const cached = activeProgressCache.get(card.id) || {
        correct_count: 0,
        incorrect_count: 0,
      };
      ...
      activeProgressCache.set(card.id, {
        ...
        incorrect_count: !correct
          ? cached.incorrect_count + 1
          : cached.incorrect_count,
      });
    ...
    [saveProgressBackground, saveSession, setCurrentIndex, updateStats],
  );
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4">
        <div className="text-center">
          ...
          <p className="mb-6 text-muted-foreground">
            Não há cards pendentes para revisar agora.
          </p>
          <button
  ...
  const progressPercent =
    totalCardsCount > 0 ? (sessionStats.reviewed / totalCardsCount) * 100 : 0;
  const currentCardNumber =
    totalCardsCount > 0
      ? Math.min(sessionStats.reviewed + 1, totalCardsCount)
      : 0;
  ...
  return (
    <div className="flex h-[100dvh] flex-col bg-background overflow-hidden">
      ...
      <div className="w-full shrink-0 h-[1px] bg-border">
        <div
          className="h-full transition-all duration-300 ease-out bg-muted-foreground/30"
      ...
      {/* Header */}
      <div
        className="grid shrink-0 pt-safe items-center px-5 pt-4 pb-2"
        style={{ gridTemplateColumns: "40px 1fr 40px" }}
      >
        <button
          onClick={handleExit}
          className="justify-self-start p-1 transition-colors active:scale-95 touch-manipulation text-muted-foreground/40"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        >
          ...
          />
          <span
            className={`font-mono text-[15px] font-normal tracking-wide transition-colors duration-300 ${stopwatchRunning ? "text-foreground/85" : "text-muted-foreground/40"}`}
          >
      ...
      {/* Card area */}
      <div
        className="flex flex-1 items-center justify-center py-2 sm:py-4 min-h-0"
        style={{ paddingBottom: flipped ? 120 : 0 }}
      >
        <div
        ...
        >
          <CardDisplay
            ...
            flipped={flipped}
            onFlip={handleFlip}
            onSwipe={flipped ? handleSwipe : undefined}
          />
        </div>
      </div>
      ...
      {/* Rating buttons */}
      {flipped && (
        <RatingButtons onRate={handleRating} disabled={transitioning} />
      )}
